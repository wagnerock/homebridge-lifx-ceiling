...
const { normalizeMac, indicesForRegion, CEILING_UPLIGHT_CELLS } = require('./matrix');
...
/**
 * Describe the region that is actually in force, e.g. `4 corner cells [0,7,56,63]`.
 * Resolving is pure maths; an unresolvable value degrades to a labelled string rather
 * than throwing, because this runs on the startup path.
 */
function describeRegion(region, width = 8, height = 8, showList = true) {
  let idx;
  try {
    idx = indicesForRegion(region, width, height);
  } catch (e) {
    const shown = Array.isArray(region) ? `[${region.slice(0, 8).join(',')}${region.length > 8 ? ',…' : ''}]` : String(region);
    return `${shown} (unresolved: ${e.message})`;
  }
  const corners = idx.length === CEILING_UPLIGHT_CELLS.length && CEILING_UPLIGHT_CELLS.every((i) => idx.includes(i));
  if (!showList) return `${idx.length} ${corners ? 'corner ' : ''}cells`;
  return `${idx.length} ${corners ? 'corner ' : ''}cells [${idx.slice(0, 8).join(',')}${idx.length > 8 ? ',…' : ''}]`;
}
...
class LifxCeilingPlatform {
  constructor(log, config, api) {
    ...
    this.config = config || {};
    this.api = api;
    this.mapping = new Mapping(this.config.mappingFile, this.log);
    this.aliases = this._readAliases(this.config.aliases);
    // The uplight/downlight geometry is a constant, not a setting — there is
    // deliberately no uplightRegion/downlightRegion knob. See matrix.CEILING_UPLIGHT_CELLS.
    this._logMappingInForce();
    this.lifx = null;
  ...
  /** One line every boot naming the geometry in force and who set it. */
  _logMappingInForce(width = 8, height = 8) {
    this.log(`mapping: uplight=${describeRegion(this.mapping.data.uplight, width, height)} `
      + `downlight=${describeRegion(this.mapping.data.downlight, width, height, false)} `
      + `source=${this.mapping.globalSource || 'default'}`);
    return this.mapping;
  }
  ...
  async start() {
    ...
    this.lifx = new Lifx();
    this.log(`homebridge-lifx-ceiling starting (uplight=${this.mapping.data.uplight} downlight=${this.mapping.data.downlight})`);
    try {
      await this.discover();
    } catch (e) {
  ...
  async discover() {
    ...
    for (const device of Array.isArray(found) ? found : []) {
      ...
      /*
       * HOMEKIT IDENTITY IS THE FIXTURE IP — permanently, not by omission.
       *
       * Bridged accessories cannot be unpaired individually in Home.app; only the
       * whole bridge can be removed. Re-minting these UUIDs would therefore strand
       * eight unremovable "No Response" tiles and force dropping a bridge that also
       * carries non-LIFX accessories. The cached context.stableId / context.mac in
       * accessories/cachedAccessories are IP-derived, so this token must keep
       * matching them byte-for-byte: restored accessories are then reused, not
       * duplicated.
       *
       * Consequence: every fixture needs a DHCP reservation (README → "Identity and
       * pairing"). A new lease means a new tile pair.
       *
       * Moving to hardware-MAC identity is a deliberate, user-approved operation
       * that requires removing and re-adding the bridge in Home.app — never a side
       * effect of a refactor. context.hwMac records the real MAC now so that
       * migration has its data when the time comes.
       */
      ...
      fixture = new Fixture({
        device, mapping: this.mapping, log: this.log,
  ...
  /**
   * IP drift detection — the one robustness feature that makes IP-based identity
   * safe for every user, and a log warning ONLY.
   *
   * Identity is the fixture IP (see discover()), so a fixture that renews onto a
   * new address publishes a SECOND pair of tiles while the old pair is still in
   * Home.app. Bridged accessories cannot be unpaired individually, so the user
   * cannot clean up the duplicate from the app — the fix belongs in the router.
  ...
  configureService(accessory, half, fixture, displayName, identity, nameIsOurs = false) {
    ...
    // second one. Never derive this from fixture.mac — that is the mapping key.
    /*
