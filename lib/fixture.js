 ...
 *
 * Power: HomeKit "off" for a half is that half's brightness 0. Fixture power is
...
/** An off pixel — the frame starts here so an untouched cell is genuinely dark. */
const blank = () => ({ hue: 0, saturation: 0, brightness: 0, kelvin: 3000 });

class Fixture {
  /** @param {object} args { device, log, kelvinMin, kelvinMax } */
  constructor({ device, log, kelvinMin, kelvinMax }) {
    ...
    this.ip = device.ip;
    // Canonical HARDWARE MAC (12 lowercase hex) — the fixture's identity key, nothing else.
    // Falling back to device.ip *first* is what made a MAC-keyed config entry dead code,
    // so discovery `device.mac` (uppercase colon) is normalised first and the IP stays the
    // transport.
    //
    // This field is NOT HomeKit identity. Identity is `identity` in lib/platform.js, which
    // mints the accessory UUID, the stableId and the service subtype — none of those may be
    // derived from an address, because bridged accessories cannot be unpaired individually and
    // re-minted UUIDs would strand them. null when the device reports no MAC (a MAC here is
    // never guessed from an IP).
   ...
   * normalised lowercase form used only for identity lookups.
  ...
  async flush() {
    ...
    this._writing = true;
    try {
      // The geometry is a constant, not a lookup: the four corners are the uplight, the
      // other 60 cells are the panel. Both halves address the SAME 64-pixel buffer.
      const uplight = matrix.CEILING_UPLIGHT_CELLS.filter((i) => i < this.width * this.height);
      const downlight = matrix.ceilingsDownlightCells(this.width, this.height);
      let buf = Array.from({ length: this.width * this.height }, blank);
      buf = matrix.paintCells(buf, uplight, this._deviceColor('up'), this.width, this.height);
      buf = matrix.paintCells(buf, downlight, this._deviceColor('down'), this.width, this.height);
